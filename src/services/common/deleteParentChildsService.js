...
const DeleteParentChildeService = async (
  req,
  ParentModel,
  ChildeModel,
  JoinPropertyName
  ...
  try {
    ...
    let ChildeCreation = await ChildeModel.deleteMany(ChildeObj).session(
      session
    );
    // Second Database Process
    let ParentCreation = await ParentModel.deleteMany(ParentObj).session(
      session
    );
